  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-white">Food Identification</h1>
          <p className="text-sm text-gray-400 mt-1">
            Welcome{user?.username ? `, ${user.username}` : ""}. Upload images, then select and analyze.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={selectedIds.length === 0 ? "Select at least one image" : "Analyze selected images"}
          >
            {busy ? (
              <span className="inline-flex items-center gap-2"><span className="h-4 w-4 animate-spin inline-block rounded-full border-2 border-white/60 border-t-transparent"></span>Analyzing…</span>
            ) : (
              "Analyze Selected"
            )}
          </button>
      ...
      <div className="mt-5">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="shrink-0 w-12 h-12 rounded-xl bg-indigo-500/20 flex items-center justify-center text-lg">📷</div>
            <div className="flex-1">
              <div className="text-white font-medium">Upload images</div>
              <div className="text-xs text-gray-400">Click to choose or drag & drop JPG/PNG. Up to 50 items listed.</div>
              {localPreviews.length > 0 && (
                <div className="text-xs text-emerald-300 mt-1">{localPreviews.length} file(s) selected</div>
              )}
            </div>
        ...
        {localPreviews.length > 0 && (
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {localPreviews.map((p, i) => (
              <div key={i} className="rounded-xl border border-white/15 bg-white/5 p-2">
                <img src={p.url} alt={p.name} className="w-full h-28 object-cover rounded" />
                <div className="mt-2 text-xs text-gray-300 truncate" title={p.name}>{p.name}</div>
              </div>
            ))}
          </div>
        )}
              ...
              return (
                ...
                >
                  <div className="relative">
                    ...
                    />
                    <div className={`absolute top-2 right-2 rounded-full px-2 py-1 text-[10px] font-semibold ${checked ? "bg-indigo-500 text-white" : "bg-black/40 text-gray-200"}`}>
                      {checked ? "Selected" : "Select"}
                    </div>
                  </div>
                  <div className="mt-2 text-[12px] text-gray-300 flex flex-col gap-0.5">
                    <div className="truncate" title={it.original_name}>{it.original_name}</div>
                    <div className="text-gray-400">{formatBytes(it.size)} · {formatDate(it.created_at)}</div>
                  </div>
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={checked}
                    onChange={(e) => setSel((s) => ({ ...s, [it.image_id]: e.target.checked }))}
                  />
                </label>
              );
            })}
